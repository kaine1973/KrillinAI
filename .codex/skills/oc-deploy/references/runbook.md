...
6. `target=all` 的成功标准：verify、三个 package jobs、Linux package 和 `candidate-ready` 成功；用户附件、KrillinAI 附件及 `opencreator-release-candidate-<commit>` 身份凭据存在；publish skipped；tag 和 Release 未变化。
