...
8. release 只能对已经通过上述门禁和完整 candidate 的精确 commit 创建 tag。tag workflow 必须提升该 candidate 的不可变 artifacts，不得重新执行付费打包。不得在失败后盲目移动 tag；先本地修复、重新通过门禁、master CI 和完整 candidate，再按用户明确授权处理 tag。
