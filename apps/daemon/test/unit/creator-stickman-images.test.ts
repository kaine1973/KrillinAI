...
describe('stickman scoped images', () => {
  ...
  it('uses the portrait canvas and materializes Codex-native reference paths for Shorts', async () => {
    const { db, repository, templates, service } = setup();
    const job = service.createJob({
      projectId: 'p1',
      templateId: 'stickman-video',
    ...
    const runner = createCreatorStageRunner({
      ...
      executors: [createStickmanImageExecutor({
        ...
        async generate(request, _config, options) {
          ...
          submittedPaths = options?.codexNativeImagePaths ?? [];
          return {
            model: 'codex-native',
    ...
    expect(submittedPaths).toHaveLength(1);
    expect(submittedPaths[0]).not.toBe(referencePath);
    expect(readFileSync(submittedPaths[0]!)).toEqual(reference);
    const generated = service.getJob(job.id)!.artifacts.find(artifact => artifact.kind === 'shot_image');
