  ...
  function startWorkflow() {
    ...
    if (imageConfigurationStatus !== 'configured') {
      setNotice(imageConfigurationStatus === 'unsupported'
        ? l('当前生图服务不支持角色参考图，请切换到 Codex（订阅）、OpenAI 或 Gemini', 'The current image provider does not support character references. Switch to Codex (subscription), OpenAI, or Gemini in AI Services.')
        ...
        <div className="stickman-tts-configuration" role="status">
          ...
          <div>
            ...
            <small>{props.imageConfigurationStatus === 'unsupported'
              ? props.l('火柴人必须把所选角色图片随每个镜头提交，请切换到 Codex（订阅）、OpenAI 或 Gemini。', 'Stickman generation must attach the selected character to every shot. Switch to Codex (subscription), OpenAI, or Gemini.')
